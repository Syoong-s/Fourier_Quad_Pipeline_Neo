...
| `PreprocsType` | `int` | `2` | N/A — Lite keeps its fixed preprocessing path | No | 1 = historical F77 background/sigma estimators; 2 = current validated C++ estimators | Selects only the Standard Stage-1 estimator while preserving RuntimeConfig geometry, mask handling, and the normalized downstream coefficient contract. | Use 1 only for controlled F77-compatibility comparisons. | Yes |
...
| `PsfGroupingType` | `int` | `4` | N/A — Lite keeps its fixed adaptive path | No | 1 = independent historical F77 selection; 2 = threshold graph; 3 = mutual KNN; 4 = adaptive pair/fraction cuts | Selects Standard Stage-5 grouping. Type 1 excludes each numerically unsafe candidate individually, applies the F77 size/chi/largest-group policy to the compact safe population, builds the mandatory all-selected-star fit cache, and skips only PRESS rejection; Type 4 is the pre-migration default. | Change for controlled algorithm comparison. | Yes |
...
| `psf_pair_chi_valid_peak_fraction` | `double` | `exp(-1)` | same | No | `(0, 1)` | Strict relative-height threshold for valid adaptive pair-chi peaks. | Tune only with pair-histogram validation. | Yes |
| `psf_bad_fraction_valid_peak_fraction` | `double` | `0.10` | same | No | `(0, 1)` | Strict relative-height threshold for valid adaptive bad-fraction peaks. | Tune only with fraction-histogram validation. | Yes |
...
| `NstampType` | `int` | `2` | N/A — Lite keeps its fixed physical blank-stamp path | No | 1 = deterministic historical F77 outer-ring blank stamp; 2 = QC/random physical blank stamp; 3 = local covariance power | Selects the complete Standard Stage-3 producer/Stage-4/6 consumer contract. Types 1 and 2 are real-space stamps transformed downstream; Type 3 is already Fourier power. Type 2 preserves this repository's pre-migration default. | Change for controlled noise-method runs. | Yes |
...
|---|---|---|---|---|
| `NON_CHIP_BASE_DIRECTORIES` | `std::array<const char*, 16>` | `science`, `dqmask`, `stamps`, `expolists`, `result`, `stamps/dat_StarInfo`, `stamps/svg_StarLocus`, `stamps/fits_StarP`, `stamps/fits_PsfSrc`, `stamps/dat_ExpoInfo`, `stamps/dat_StarComp`, `stamps/dat_Rescale`, `stamps/dat_Pcs`, `stamps/dat_StarCompV2`, `astrometry/Head`, `astrometry/dat_Chk` | Complete fixed base-directory contract created without a chip suffix. | Yes |

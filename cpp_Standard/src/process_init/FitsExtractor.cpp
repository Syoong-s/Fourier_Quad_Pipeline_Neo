...
// Method: Advance past each replacement so the configured DQ mapping is exact.
// ==========================================
...
std::vector<PlannedImage> planImages(fitsfile* input,
    ...
    for (int hdu_number = 1; hdu_number <= hdu_count; ++hdu_number) {
        ...
        int ccdnum = 0;
        int key_status = 0;
        fits_read_key(input, TINT, InitConfig::CCDNUM_KEYWORD,
        ...
        if (key_status != 0) {
            throw std::runtime_error(
                "cannot read " + std::string(InitConfig::CCDNUM_KEYWORD)
...
std::string archiveStem(const std::filesystem::path& source) {
    ...
    const std::size_t suffix_length = std::strlen(InitConfig::ARCHIVE_SUFFIX);
    if (filename.size() <= suffix_length
        || filename.compare(filename.size() - suffix_length,
                            suffix_length, InitConfig::ARCHIVE_SUFFIX) != 0) {
        throw std::runtime_error(
            "archive does not end in " + std::string(InitConfig::ARCHIVE_SUFFIX)
            + ": " + source.string());
    }
...
std::string dqOutputStem(const std::filesystem::path& source) {
    ...
    replaceAll(stem, InitConfig::DQ_STEM_REPLACE_FROM,
               InitConfig::DQ_STEM_REPLACE_TO);
    return stem;
}

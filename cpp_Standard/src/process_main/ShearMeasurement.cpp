...
// Method: Apply the shared norm gate before chip products, then mirror the F77 PSF evaluation,
//         Fourier_Quad estimator, and catalog write for valid chips.
// ==========================================
    ...
    for (int ichip = 0; ichip < nchip; ++ichip) {
                ...
                if (nx < step_psf || ny < step_psf) {
                    ...
                    if (validity_marker < -1.0f) {
                        ...
                        std::cerr << "Error / successful local PSF geometry "
                                  << nx << "x" << ny << " does not match runtime CCD "
        ...
        for (int i = 0; i < ngal; ++i) {
            ...
            if (!bad_source) {
                ...
                if (!bad_source) {
                    gal_para[i][LensingConfig::igalsizeT] =
                        measurePowerCurvatureSize(
                            ns, gal_p, LensingConfig::size_fit_rmax);
                    gal_para[i][LensingConfig::ipsfsizeT] =
                        measurePowerCurvatureSize(
                    ...
                    gal_para[i][LensingConfig::idelta_chi2] =
                        point_stats.delta_chi2;
            ...
            for (int j = 0; j < LensingConfig::shear_cat_ncols; ++j) {
                ...
                fout10 << (j + 1 == LensingConfig::shear_cat_ncols ? "" : " ");
            }
            fout10 << "\n";
        }
        fout10.close();

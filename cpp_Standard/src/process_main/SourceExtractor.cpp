...
namespace SourceExtractor {
    ...
    //         identity from the canonical filename before the configured catalog branch.
    // ==========================================
    void chipProcessSource(const std::string& imageFile, const std::string& dirOutput) {
        ...
        if (lensing.include_flat == 1) {
            const int cid = UniversalUtils::getChipId(imageFile);
            std::ostringstream oss;
            oss << std::setw(2) << std::setfill('0') << cid;
            const std::string flat_file = lensing.flat_path + "/flat_"
        ...
        if (LensingConfig::include_BGsub == 1) {
            subtractBackground(nx, ny, array, bg_coeffs, lensing.ccd_split,
        ...
        for (int amp = 0; amp < lensing.ccd_split; ++amp) {
        ...
        if (lensing.ext_cat == 0) {
            genSourceCatalog(
                dirOutput, PREFIX, nx, ny, array, weight, sigmap, ngal, proc_error);
            ...
            if (lensing.ext_psf == 0) {
                genStarCandidateDirect(
                    dirOutput, PREFIX, nx, ny, array, weight, sigmap,
                    nstar, proc_error);
            }
        }
    ...
    // ==========================================
    // Function: Publish detected source stamps and chip source metadata
    // Method: Consume the temporary detection catalog, use the Stage-1 local-sigma map for noise
    //         candidates, and route all text/FITS products through checked main-process writers.
    // ==========================================
    void genSourceCatalog(const std::string& dirOutput, const std::string& prefix, int nx, int ny, const std::vector<float>& array,
                          std::vector<int>& weight, const std::vector<float>& sigmap,
                          int& ngal, int& procError) {
        ngal = 0;
        ...
        if (procError != 1) {
            ...
            if (!fin.is_open()) {
                MPIFailure::abortWorld("read source catalog", catname);
            }
    ...
    // Method: Apply fixed geometry, runtime amplifier, mask, local-sigma, MAD, and positive-tail
    //         gates; shuffle candidates with the existing rank-local ran1 stream and accept the
    //         first candidate that passes complete post-flatten quality control.
    // ==========================================
    void findNoise(int& flag, std::vector<float>& stamps, int nx, int ny, const std::vector<float>& array,
        ...
        const int nxc = nx / 2;
        const int ccdSplit = RuntimeConfigStore::get().lensing.ccd_split;
    ...
    // ==========================================
    // Function: Produce the current QC/random blank-noise and source-stamp pair
    // Method: Preserve the modern branch's rejection and RNG order by finding
    //         noise before checking the source.
    // ==========================================
    void BlankSrcStamp(
    ...
    void CovarSrcStamp(
        ...
        if (!NoisePlaneFit::fitNoiseRegionPlane(
                ...
                sourceOffset, localStartX, localStartY, nx, ny, ccdSplit, initialCenterX,
        ...
        if (NoiseCovariance::sourceStampCrossesAmplifier(
                nx, ccdSplit, sourceStampStartX, LensingConfig::ns)) {
            flag = -1;
            return;
        }
    ...
    // ==========================================
    // Function: Publish star candidates selected from the source catalog
    // Method: Filter Stage-3 sources by SNR and route candidate metadata and
    //         stamp products through checked main-process writers.
    // ==========================================
    void genStarCandidate(const std::string& dirOutput, const std::string& prefix, int& nstar, int& procError) {
        ...
        if (procError != 1) {
            ...
            if (!fin.is_open()) {
                MPIFailure::abortWorld("read source info for star candidates",
                                       filename);
            }
    ...
    // Method: Re-extract qualifying detections through the selected source/noise path and route
    //         candidate text/FITS products through checked writers.
    // ==========================================

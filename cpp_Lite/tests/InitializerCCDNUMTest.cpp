...
enum class KeyKind { Missing, Integer, Malformed };
...
void appendImage(fitsfile* file, KeyKind key_kind, int ccdnum) {
    ...
    float pixels[4] = {static_cast<float>(ccdnum), 1.0f, 2.0f, 3.0f};
...
void createArchive(
    ...
    for (const auto& extension : extensions) appendImage(file, extension.first, extension.second);
...
int readCCDNUM(const std::filesystem::path& path) {
    ...
    int ccdnum = 0;
    if (status == 0) fits_read_key(file, TINT, "CCDNUM", &ccdnum, nullptr, &status);
...
void replaceCCDNUM(const std::filesystem::path& path, int ccdnum) {
    ...
    if (status == 0) fits_update_key(file, TINT, "CCDNUM", &ccdnum, nullptr, &status);
...
void testCanonicalExtractionAndResume() {
    ...
    createArchive(science, {
        {KeyKind::Integer, 7}, {KeyKind::Missing, 0},
        {KeyKind::Integer, 1}, {KeyKind::Integer, 3}});

    const std::filesystem::path final_dir = tree.root() / "science";
    fqinit::ExtractionResult result = fqinit::extractArchive(
        science, fqinit::ProductKind::Science, final_dir,
        tree.root() / "stage_science", fqinit::ExistingPolicy::Overwrite);
    require(result.success && result.output_paths.size() == 3,
    ...
    require(dq_result.output_paths.front().filename()
                == fqinit::dqOutputStem(science) + "_7.fits",
